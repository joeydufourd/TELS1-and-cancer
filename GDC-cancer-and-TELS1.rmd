...
date: 'Recreated the 12th September, 2025 (from 2021 analysis) (&nbsp;Last update on the `r paste(format(Sys.time(), "%d"), month.name[as.integer(format(Sys.time(), "%m"))], format(Sys.time(), "%Y"))` )'
output:
    html_document:
        theme: cosmo
        toc: false
...
cran_packages <- c(
  ...
  "plotly", "ggforce", "ggrepel", "DT", "shiny"
)
...
n <- length(unique(merged_clinical$primary_diagnosis))
mycols <- viridis(n, option = "cividis")

merged_clinical %>%
  dplyr::count(primary_diagnosis) %>%
  ggplot(aes(x = "", y = n, fill = primary_diagnosis)) +
  geom_col() +
  coord_polar(theta = "y") +
